  ...
  if (localDW->is_active_c25_flightControlSystem == 0U) {
    ...
    switch (localDW->is_c25_flightControlSystem) {
     ...
     default:
      ...
      localB->z_planned = -0.7;
      localB->flag_end_marker_to_controller = 0.0;
      if ((rtu_VisionbasedData[2] == 1.0) && (localB->norm_error_tot <= 5.0)) {
...
void flightControlSystem_Init(void)
{
  ...
  /* SystemInitialize for MATLAB Function: '<S2>/Waypoints_straight' */
...
void flightControlSystem_Reset(void)
{
  ...
  /* SystemReset for MATLAB Function: '<S2>/Waypoints_straight' */
  flightControlSystem_DW.x_accumulator = 0.0;
  flightControlSystem_DW.y_accumulator = 0.0;
  flightControlSystem_DW.pixel_count = 0.0;
...
{
  real_T rtb_error_x_end_marker;
  real_T rtb_flag_marker;
  int32_T bufIdx;
  int32_T i;
  int32_T ku;
  int32_T ky;
  ...
  /* MATLAB Function: '<S2>/Waypoints_straight' incorporates:
   *  Delay: '<S2>/Delay'
   *  Delay: '<S2>/Delay1'
   *  Delay: '<S2>/Delay2'
   */
  flightControlSystem_B.center_x = mod_nOBiMHsG(rt_atan2d_snf
    (-flightControlSystem_DW.Delay_DSTATE, flightControlSystem_DW.Delay1_DSTATE));
  if (flightControlSystem_DW.Delay2_DSTATE) {
    for (ky = 0; ky < 81; ky++) {
      for (m = 0; m < 81; m++) {
        flightControlSystem_B.b_norm = ((real_T)ky + 20.0) - 59.5;
        rtb_flag_marker = ((real_T)m + 40.0) - 79.5;
        flightControlSystem_B.b_norm = sqrt(flightControlSystem_B.b_norm *
          flightControlSystem_B.b_norm + rtb_flag_marker * rtb_flag_marker);
        if ((flightControlSystem_B.b_norm >= 30.0) &&
          ...
          rtb_flag_marker = mod_nOBiMHsG(flightControlSystem_B.center_x +
            1.0833078115826873);
          rtb_error_x_end_marker = mod_nOBiMHsG(flightControlSystem_B.center_x -
            1.0833078115826873);
          if (rtb_flag_marker < rtb_error_x_end_marker) {
            flag_error_x = (((flightControlSystem_B.b_norm <= 6.2831853071795862)
                             && (flightControlSystem_B.b_norm >=
                                 rtb_error_x_end_marker)) ||
                            ((flightControlSystem_B.b_norm <= rtb_flag_marker) &&
                             (flightControlSystem_B.b_norm >= 0.0)));
          } else {
            flag_error_x = ((flightControlSystem_B.b_norm <= rtb_flag_marker) &&
                            (flightControlSystem_B.b_norm >=
                             rtb_error_x_end_marker));
          }
    ...
    for (ku = 0; ku < 81; ku++) {
      for (m = 0; m < 81; m++) {
        rtb_error_x_end_marker = ((real_T)ku + 20.0) - 59.5;
        flightControlSystem_B.b_norm = ((real_T)m + 40.0) - 79.5;
        flightControlSystem_B.b_norm = sqrt(rtb_error_x_end_marker *
          rtb_error_x_end_marker + flightControlSystem_B.b_norm *
          flightControlSystem_B.b_norm);
        if ((flightControlSystem_B.b_norm >= 30.0) &&
            (flightControlSystem_B.b_norm <= 40.0) &&
  ...
  flightControlSystem_B.x_ref_temp = -(rt_roundd_snf
  ...
  flightControlSystem_B.y_ref_temp = rt_roundd_snf
    ...
    - 80.0;
  if (rtIsNaN(flightControlSystem_B.x_ref_temp)) {
    flightControlSystem_B.x_ref_temp = 0.0;
    flag_error_x = false;
  } else {
    flag_error_x = true;
  }
  ...
  if (flightControlSystem_DW.count_pixel != 0.0) {
    i = 1;
    flightControlSystem_B.center_x = rt_roundd_snf
      (flightControlSystem_DW.row_accumulator /
       flightControlSystem_DW.count_pixel);
    flightControlSystem_B.center_y = rt_roundd_snf
      (flightControlSystem_DW.column_accumulator /
       flightControlSystem_DW.count_pixel);
  } else {
    i = 0;
    flightControlSystem_B.center_x = 60.0;
    flightControlSystem_B.center_y = 80.0;
  }

  flightControlSystem_DW.column_accumulator = 0.0;
  flightControlSystem_DW.row_accumulator = 0.0;
  flightControlSystem_DW.count_pixel = 0.0;
  rtb_error_x_end_marker = -(flightControlSystem_B.center_x - 60.0);
  flightControlSystem_B.b_norm = flightControlSystem_B.center_y - 80.0;
  rtb_flag_marker = i;

  /* End of MATLAB Function: '<S2>/Red Marker Detection' */
  /* Update for Delay: '<S2>/Delay' incorporates:
   *  MATLAB Function: '<S2>/Waypoints_straight'
   */
  flightControlSystem_DW.Delay_DSTATE = flightControlSystem_B.x_ref_temp;
  ...
  flightControlSystem_DW.RateTransition_Buffer[3 +
    (flightControlSystem_DW.RateTransition_ActiveBufIdx == 0) * 6] =
    flightControlSystem_B.x_ref_temp;
